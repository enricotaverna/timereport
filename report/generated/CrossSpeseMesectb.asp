...
' ASP Report Maker 1.0 - Table level configuration (CrossSpeseMese)
' Table Level Constants

Const EW_TABLE_VAR = "CrossSpeseMese"
Const EW_TABLE_GROUP_PER_PAGE = "grpperpage"
Const EW_TABLE_SESSION_GROUP_PER_PAGE = "CrossSpeseMese_grpperpage"
Const EW_TABLE_START_GROUP = "start"
Const EW_TABLE_SESSION_START_GROUP = "CrossSpeseMese_start"
Const EW_TABLE_SESSION_SEARCH = "CrossSpeseMese_search"
Const EW_TABLE_CHILD_USER_ID = "childuserid"
Const EW_TABLE_SESSION_CHILD_USER_ID = "CrossSpeseMese_childuserid"
...
EW_TABLE_SQL_SELECT = "SELECT [NomeProgetto], [Persona], [DescSpesa] <DistinctColumnFields> FROM " & EW_TABLE_SQL_FROM
Dim EW_TABLE_SQL_WHERE
EW_TABLE_SQL_WHERE =  session("whereclause")
Const EW_TABLE_SQL_GROUPBY = "[NomeProgetto], [Persona], [DescSpesa]"
Const EW_TABLE_SQL_ORDERBY = "[NomeProgetto] ASC, [Persona] ASC, [DescSpesa] ASC"
Const EW_TABLE_SQL_PIVOT = ""
Dim EW_TABLE_DISTINCT_SQL_SELECT, EW_TABLE_DISTINCT_SQL_WHERE, EW_TABLE_DISTINCT_SQL_ORDERBY
EW_TABLE_DISTINCT_SQL_SELECT = "SELECT DISTINCT [AnnoMese] FROM [v_spese]"
...
Dim af_ConversionRate ' Advanced filter for ConversionRate
...
Dim af_Projects_Id ' Advanced filter for Projects_Id
...
Dim x_Persona, ox_Persona, tx_Persona, gx_Persona, ftx_Persona
x_Persona = Null: ox_Persona = Null: tx_Persona = Null: gx_Persona = Null: ftx_Persona = 202
Dim rf_Persona, rt_Persona
Dim x_DescSpesa, ox_DescSpesa, tx_DescSpesa, gx_DescSpesa, ftx_DescSpesa
x_DescSpesa = Null: ox_DescSpesa = Null: tx_DescSpesa = Null: gx_DescSpesa = Null: ftx_DescSpesa = 202
...
Cross Spese Mese
<% If sExport = "" Then %>
&nbsp;&nbsp;<a href="CrossSpeseMesectb.asp?export=excel">Export to Excel</a>
<% End If %>
<br><br>
<% If sExport = "" Then %>
</div></td></tr>
...
<table id="ewReport" class="ewTable">
	<!-- Table header -->
	<tr class="ewTableRow">
		<td colspan="3" nowrap><div class="aspreportmaker">Importo (SUM)&nbsp;</div></td>
...
Do While (Not rs.Eof)
	...
	If CLng(nGrpCount) >= CLng(nStartGrp) And (nGrpCount <= nStopGrp) Then
		...
		If (x_Persona <> "" And ox_Persona = x_Persona) And _
			Not ChkLvlBreak(2) Then
			gx_Persona = ""
		ElseIf IsNull(x_Persona) Then
			gx_Persona = EW_NULL_LABEL
		ElseIf x_Persona = "" Then
			gx_Persona = EW_EMPTY_LABEL
		End If
		gx_DescSpesa = x_DescSpesa
		If (x_DescSpesa <> "" And ox_DescSpesa = x_DescSpesa) And _
			Not ChkLvlBreak(3) Then
			gx_DescSpesa = ""
		ElseIf IsNull(x_DescSpesa) Then
			gx_DescSpesa = EW_NULL_LABEL
		ElseIf x_DescSpesa = "" Then
			gx_DescSpesa = EW_EMPTY_LABEL
		End If
%>
	<!-- Data -->
	<tr<%=sItemRowClass%>>
...
<% x_NomeProgetto = tx_NomeProgetto %></td>
		<!-- Persona -->
		<td class="ewRptGrpField2"><% tx_Persona = x_Persona: x_Persona = gx_Persona %>
<%= x_Persona %>
<% x_Persona = tx_Persona %></td>
		<!-- Desc Spesa -->
		<td class="ewRptGrpField3"><% tx_DescSpesa = x_DescSpesa: x_DescSpesa = gx_DescSpesa %>
<%= x_DescSpesa %>
<% x_DescSpesa = tx_DescSpesa %></td>
<!-- Dynamic columns begin -->
	<%
	rowsmry = 0
	For iy = 1 to UBound(val)
...
<%
		...
		If ChkLvlBreak(2) Then
%>
	<!-- Summary Persona (level 2) -->
	<tr>
		<td class="ewRptGrpField1"></td>
		<% tx_Persona = x_Persona: x_Persona = ox_Persona %>
		<td  class="ewRptGrpSummary2" colspan="2">Summary for Persona: 
<%= x_Persona %>
</td>
		<% x_Persona = tx_Persona %>
<!-- Dynamic columns begin -->
	<%
	rowsmry = 0
...
%>
	<!-- Summary Nome Progetto (level 1) -->
	<tr>
		<% tx_NomeProgetto = x_NomeProgetto: x_NomeProgetto = ox_NomeProgetto %>
		<td  class="ewRptGrpSummary1" colspan="3">Summary for Nome Progetto: 
<%= x_NomeProgetto %>
</td>
		<% x_NomeProgetto = tx_NomeProgetto %>
<!-- Dynamic columns begin -->
	<%
	rowsmry = 0
	For iy = 1 to UBound(smry,2)
...
<br>
<% If sExport = "" Then %>
<form action="CrossSpeseMesectb.asp" name="ewpagerform" id="ewpagerform">
...
<!--first page button-->
	...
	<% Else %>
	<td><a href="CrossSpeseMesectb.asp?start=1"><img src="rptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<% End If %>
<!--previous page button-->
	<% If CLng(PrevStart) = CLng(nStartGrp) Then %>
	<td><img src="rptimages/prevdisab.gif" alt="Previous" width="16" height="16" border="0"></td>
	<% Else %>
	<td><a href="CrossSpeseMesectb.asp?start=<%=PrevStart%>"><img src="rptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<% End If %>
<!--current page number-->
...
<!--next page button-->
	...
	<% Else %>
	<td><a href="CrossSpeseMesectb.asp?start=<%=NextStart%>"><img src="rptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>
	<% End If %>
<!--last page button-->
	<% If CLng(LastStart) = CLng(nStartGrp) Then %>
	<td><img src="rptimages/lastdisab.gif" alt="Last" width="16" height="16" border="0"></td>
	<% Else %>
	<td><a href="CrossSpeseMesectb.asp?start=<%=LastStart%>"><img src="rptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>
	<% End If %>
...
Sub GetColumns()
	...
	Redim col(2,ncol), val(ncol), oval(ncol), cnt(3,ncol), smry(3,ncol), grandsmry(ncol)
	...
	For colcnt = 1 to ncol
		sFld = ew_CrossTabField("SUM", "[Importo]", "[AnnoMese]", "", col(0,colcnt), "'", colcnt)
...
Function ChkLvlBreak(lvl)
	Select Case lvl
		...
		Case 3: ChkLvlBreak = _
			(IsNull(x_DescSpesa) And Not IsNull(ox_DescSpesa)) Or _
			(Not IsNull(x_DescSpesa) And IsNull(ox_DescSpesa)) Or _
			(x_DescSpesa <> ox_DescSpesa) Or ChkLvlBreak(2) ' Recurse upper level
	End Select
End Function
